...
import { companies, inventoryBalances, products, warehouses } from './schema/index.js';
...
const [salesVanWarehouse] = await db
  ...
  .onConflictDoUpdate({
    target: [warehouses.companyId, warehouses.code],
    set: { name: 'Gudang Sales Van 001', status: 'active' },
  })
  .returning();

console.log(`✅ Gudang Utama : ${mainWarehouse?.name}`);
console.log(`✅ Gudang Sales : ${salesVanWarehouse?.name}`);
